...
const App = () => {
  ...
  const [formData, setFormData] = useState({ date: '', category: '', amount: '', type: 'income' });
  ...
  const fetchTransactions = () => {
    ...
    const dummyTransactions = [
      { date: '2022-02-15', category: 'Groceries', amount: 50.0, type: 'expense' },
      { date: '2022-02-14', category: 'Salary', amount: 2000.0, type: 'income' },
      ...
      {!loggedIn ? (
        <div id="loginForm">
          ...
          <form>
            ...
            <button type="button" onClick={() => setLoggedIn(true)} className="loginBtn">Login</button>
          </form>
        </div>
      ) : (
        <div>
          <div id="transactionForm">
            ...
            <form>
              ...
              <label>Amount:</label>
              <input type="number" onChange={(e) => setFormData({ ...formData, amount: e.target.value })} required />
              ...
              <button type="button" onClick={addTransaction} className='addTransactionBtn'>Add Transaction</button>
            </form>
          </div>
            ...
            {transactions.length === 0 ? (
              ...
              <ul>
                {transactions.map((transaction, index) => (
                  <li key={index}>
                    Date: {transaction.date} - Category: {transaction.category} - Amount: ${transaction.amount.toFixed(2)} - Type: {transaction.type}
                  </li>
                ))}
              </ul>
            )}
